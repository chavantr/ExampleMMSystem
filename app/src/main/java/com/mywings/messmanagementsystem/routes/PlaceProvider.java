...
public class PlaceProvider extends ContentProvider {
    ...
    public static final String AUTHORITY = "in.wptrafficanalyzer.locationsearchdialogv2kisok.PlaceProvider";
    ...
    String mKey = "key=AIzaSyA1fYR6-7DIhgORWbFju3nGi3BDojCILp8";
        ...
        switch (mUriMatcher.match(uri)) {
            case SEARCH:
                ...
                try {
                    ...
                    // Finding latitude and longitude for each places using Google
                    // Places Details API
                    for (int i = 0; i < list.size(); i++) {
                        ...
                        detailsList = detailsParser.parse(new JSONObject(
                                jsonPlaceDetails));

                        // Creating cursor object with places
                        for (int j = 0; j < detailsList.size(); j++) {
                            HashMap<String, String> hMapDetails = detailsList
                                    .get(j);

                            // Adding place details to cursor
                            mCursor.addRow(new String[]{hMap.get("description"),
                                    hMapDetails.get("lat"), hMapDetails.get("lng")});
                        }
            ...
            case DETAILS:
                ...
                for (int j = 0; j < detailsList.size(); j++) {
                    ...
                    mCursor.addRow(new String[]{
                            hMapDetails.get("formatted_address"),
                            hMapDetails.get("lat"), hMapDetails.get("lng")});
                }
                c = mCursor;
                break;
    ...
    private String getPlacesUrl(String qry) {
        ...
        // Building the parameters to the web service
        String parameters = qry + "&" + types + "&" + mKey + "&" + sensor;
